...
(function () {
const PLUGIN_ID = "readable-output";
const DISPLAY_NAME = "看得清 Readable Output";
...
function resolveLanguageProfile(text, settings) {
...
function readabilityScore(text, settings) {
  const sample = typeof text === "string" ? text : "";
  if (!sample.trim()) {
    return 0;
  }
...
function rewriteReadableOutput(text, settings) {
...
function collectAssistantTexts(message) {
  if (!isRecord(message) || message.role !== "assistant" || !Array.isArray(message.content)) {
    return [];
  }
...
function createRecentlyRewrittenCache(ttlMs = 60_000) {
  const cache = new Map();
